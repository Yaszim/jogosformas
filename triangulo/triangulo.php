...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (($soma - 180) < 0.001) {
        ...
        if($lado1 === $lado2 && $lado2 === $lado3){
            ...
            $triangulo = new Equilatero($id, $lado1, $lado2, $lado3, $tipo, $cor, $un, $destino);
        }
        elseif($lado1 === $lado2 || $lado2 === $lado3 || $lado3 === $lado1){
            $tipo = "Isóceles";
            $un = UnidadeMedida::listar(1, $un)[0];
            $triangulo = new Isoceles($id, $lado1, $lado2, $lado3, $tipo, $cor, $un, $destino);
        }
        else{
            $tipo = "Escaleno";
            $un = UnidadeMedida::listar(1, $un)[0];
            $triangulo = new Escaleno($id, $lado1, $lado2, $lado3, $tipo, $cor, $un, $destino);
        }
        try {
            $resultado = "";
            switch ($acao) {
            ...
            if ($resultado)
                header('Location: index.php?Os_valores_fornecidos_formam_um_triângulo_válido');
            else
        ...
        header('Location: index.php?Os_valores_fornecidos_não_formam_um_triângulo_válido.');
    }
